...
/**
 * Буквы групп для показа пользователю — кириллические А/Б/В.
 * id группы в STRUCT остаётся латинским (A/B/C): он служит ключом i18n и TS-типом.
 * Здесь — только то, что видно на бейдже редактора и в колонке «Гурӯҳ» Excel.
 */
export const GROUP_LETTER: Record<'A' | 'B' | 'C', string> = { A: 'А', B: 'Б', C: 'В' };
...
const STRUCT: GroupStruct[] = [
  ...
  },
  {
    ...
    indicators: [
      ...
      { key: 'k14', code: 'K14', scores: [10, 6, 4, 2, 0] },
      { key: 'k15', code: 'K15', scores: [10, 6, 4, 2, 0] },
